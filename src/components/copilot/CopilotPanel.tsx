...
const CopilotPanel = () => {
  ...
  const sendMessage = useCallback(async (message: string, aiMessageId: number) => {
    try {
      const response = await fetch("http://127.0.0.1:8000/chat", {
        ...
        body: JSON.stringify({ message, session_id: "vinay-session" }),
      });
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === aiMessageId
            ? { ...msg, content: "⚠️ Failed to load response from backend." }
            : msg
        )
      );
  ...
  const handleSend = useCallback(() => {
    ...
    sendMessage(trimmed, aiMessageId);
  }, [input, sendMessage]);
